...
async def chat_generator(user_text: str, scene: str, conv_id: str, user_id: int):
    ...
    try:
        ...
        if keyword in ["我想看一家企业的精准画像", "我想生成一份区域经济报告", "我想查看行业发展趋势报告",
                       "我想找一些高潜客户线索"]:
            keyword = ""
        ...
        # C. 记录系统事件日志
        db.log_event(user_id, resolved_scene, "INFO",
                     f"接收到提问，RAG实体提取: '{keyword}'，解析意图场景: '{resolved_scene}'")
        ...
        if resolved_scene == "customer":
            ...
            if res.get("type") == "html_link":
                ...
                guide_text = res.get(
                    "summary") or f"已为您生成 **{res.get('district', '')}** 商机分析，请点击下方按钮查看明细。"
