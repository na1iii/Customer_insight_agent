...
def handle(keyword: str, user_id: int = None) -> dict:
    ...
    try:
        ...
        prompt = (
            ...
            f"2. **二、近期舆情动态**：根据检索上下文中提供的舆情动态与新闻按时间倒序展现（最多只显示最新的前五条）。每一条动态需包含：发布日期、主题/标题、数据源、内容摘要。注意：如果检索上下文中没有提供任何关于该企业的舆情动态/新闻，请在此部分直接且仅输出“*本系统暂未收录该企业的近期舆情动态。*”。绝对不得编造或使用你自身的知识库生成任何虚构的动态。\n"
